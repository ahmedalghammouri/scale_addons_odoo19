...
export class WeighingOverviewDashboardIn extends Component {
    setup() {
    ...
    async loadData() {
        ...
        try {
            const data = await this.orm.call("weighing.overview.incoming", "get_overview_data", []);
    ...
    async onCardAction(actionName) {
        const actions = {
            'receipts_to_weigh': {
                ...
                view_mode: 'list,form',
                domain: async () => {
                    const ids = await this.orm.call('weighing.overview.incoming', 'get_receipts_to_weigh_ids', []);
                    return [['id', 'in', ids]];
                }
            },
            ...
            'all_records': {
                name: 'All Incoming Records',
                res_model: 'truck.weighing',
                view_mode: 'list,form',
                domain: [['operation_type', '=', 'incoming']]
            }
        };
